...
import copy, math, os, argparse, re, string
...
""",
    ...
    naver = 1
    parser.add_argument("-aver", "--average", help="Output running average over N data points, number must be odd. Default "+str(naver), type=int, default=naver)
    nskip = 0
    parser.add_argument("-skip", "--skip", help="Skip this many data points when plotting. Default "+str(nskip), type=int, default=nskip)
    parser.add_argument("-paver", "--printaver", help="Print the average and standard deviation of all data sets. Can be combined with -xmin and -xmax", action="store_true")
    ...
    parser.add_argument("-panels", "--panels", help="Generate different panels to plot in, one file per panel. Add 'side' for side-by-side panels. Alternatively, specify 'AxB' (e.g., '1x2') for A rows and B columns, distributing files evenly across panels.", nargs="?", const="top", default=None)
    subfigx = -0.15
    subfigy = 1.0
    ...
    parser.add_argument("-nofilelegends", "--nofilelegends", help="Ignore legends defined in the input file(s). When combined with -dslegends only the user-defined legends are shown.", action="store_true")
    ...
    col_x = 0
    parser.add_argument("-colx", "--colx", help="Choose what x column you would like from a csv or xvg file. Choose only one column. Default is column "+str(col_x), type=int, default=col_x)
    col_y = 3
    parser.add_argument("-coly", "--coly", help="Choose what y column(s) you would like from a csv or xvg file. For xvg files, columns can be specified by integer index, legend name (e.g. EPOT), or series label (e.g. s10). Default is column "+str(col_y), nargs="+", type=str, default=str(col_y))
    ...
    parser.add_argument("-quiet", "--quiet", help="Suppress the citation message at startup", action="store_true")
...
class APIArguments:
    ...
    def __init__(self, **kwargs):
        ...
        self.average = kwargs.get("average", kwargs.get("aver", 1))
        self.skip = kwargs.get("skip", 0)
        self.printaver = kwargs.get("printaver", False)
        ...
        self.nofilelegends = kwargs.get("nofilelegends", False)
...
def _resolve_xvg_col(v, legends):
    """Resolve a user-supplied column specifier for an xvg file.

    In xvg data lines, column 0 is the x/time axis and column k+1 holds series s(k)
    (i.e. the series whose legend is legends[k]).

    Accepts:
    - sN string (e.g. 's10'): series N, which occupies data column N+1
    - legend name (case-insensitive match against ``legends``): resolves to the
      corresponding 1-based data column index
    - integer string or int: used directly as 0-based column index into the data line

    Returns the resolved integer column index, or None if resolution failed.
    """
    s = str(v)
    if len(s) > 1 and s[0] == 's' and s[1:].isdigit():
        return int(s[1:]) + 1
    for idx, leg in enumerate(legends):
        if leg.lower() == s.lower():
            return idx + 1
    try:
        return int(s)
    except ValueError:
        print(f"Warning: Cannot resolve xvg column '{s}', ignoring.")
        return None
...
def read_xvg(args, filename:str, residual:bool=False, filelabel:bool=False):
    ...
    numwords = None
    col = []
    xvg_col_fixed = False   # True once xvg column selection has been resolved
    xvg_custom_col = False  # True when user explicitly selected columns for xvg
    ...
    with open(filename, "r") as inf:
        ...
        for line in inf:
            ...
            if xvg is False:
                ...
                w = line.split()
                if not xvg_col_fixed:
                    # Resolve xvg column selection once, on the first data line.
                    # Detect whether the user supplied explicit column values.
                    coly_is_default = args.coly in ("3", ["3"])
                    colx_is_default = (args.colx == 0)
                    if not coly_is_default or not colx_is_default:
                        coly_list = args.coly if isinstance(args.coly, list) else [args.coly]
                        resolved_colx = _resolve_xvg_col(args.colx, legends)
                        if resolved_colx is None or resolved_colx >= len(w):
                            if resolved_colx is not None:
                                print(f"Warning: xvg colx index {resolved_colx} out of range "
                                      f"(0-{len(w)-1}), using column 0.")
                            resolved_colx = 0
                        col = [resolved_colx]
                        for v in coly_list:
                            idx = _resolve_xvg_col(v, legends)
                            if idx is not None:
                                if 0 <= idx < len(w):
                                    col.append(idx)
                                else:
                                    print(f"Warning: xvg column index {idx} out of range "
                                          f"(0-{len(w)-1}), skipping.")
                        if len(col) < 2:
                            # No valid y-columns resolved; fall back to all columns.
                            col = [i for i in range(len(w))]
                        else:
                            # Filter legends to match only the selected y-columns.
                            # legends[k] is the legend for series s(k), which sits at
                            # data column k+1.  For a selected y-column c, the series
                            # index is c-1, so the matching legend is legends[c-1].
                            new_legends = []
                            for c in col[1:]:
                                leg_idx = c - 1
                                if 0 <= leg_idx < len(legends):
                                    new_legends.append(legends[leg_idx])
                                else:
                                    new_legends.append("")
                            legends = new_legends
                            xvg_custom_col = True
                        ...
                        col = [i for i in range(len(w))]
                    xvg_col_fixed = True
                elif not xvg_custom_col:
                    # No custom columns; reset col for every data line (original behaviour).
                    col = [i for i in range(len(w))]
                numwords = len(col)
            if numwords == 2:
                if len(dataset) == 0:
    ...
    #Perform running average if desired
    if args.average > 1:
        dx = int(args.average / 2)
        if args.average % 2 == 0:
            print("Increasing average by one to make running average symmetrical.")
            args.average += 1
        for i in range(len(dataset)):
            ycopy = np.copy(dataset[i].y)
            for j in range(len(ycopy)):
                thisdx = dx
                #In the beginning and end of the data set we do not have sufficient amount of points
                if j < dx:
                    thisdx = j
                elif j >= len(ycopy)-1-dx:
                    thisdx = len(ycopy)-1-j
                dataset[i].y[j] = np.sum(ycopy[j-thisdx:j+thisdx+1])/(2*thisdx+1)
    #Prune data sets if desired.
    if args.skip > 0:
        #See: https://stackoverflow.com/questions/25876640/subsampling-every-nth-entry-in-a-numpy-array
        for i in range(len(dataset)):
            xcopy = np.copy(dataset[i].x)
            dataset[i].x = xcopy[::args.skip]
            ycopy = np.copy(dataset[i].y)
            dataset[i].y = ycopy[::args.skip]
...
class DataSet:
    ...
    def printAverages(self, args, dataset, legends):
        for i in range(len(dataset)):
            sumy  = 0
            sum2y = 0
            ny    = 0
            for j in range(len(dataset[i].y)):
                if ((args.xmin and dataset[i].x[j] >= args.xmin or not args.xmin) and
                    (args.xmax and dataset[i].x[j] <= args.xmax or not args.xmax)):
                    sumy  += dataset[i].y[j]
                    sum2y += dataset[i].y[j]**2
                    ny += 1
            if ny > 1:
                myav  = sumy/ny
                mystd = math.sqrt(sum2y/ny - myav**2)
                myleg = ""
                if legends:
                    myleg = legends[i]
                print("Dataset %d %s average %g stddev %g" %
                      ( i, myleg, myav, mystd ) )
    ...
    def read(self, args, filenm:str, residual:bool, filelabel:bool, arglegend, setcount):
        ...
        if len(dataset) == 0:
            ...
            if args.printaver:
                self.printAverages(args, dataset, legend)
            for d in dataset:
...
def process_plot(args, fig, axs, nfiles, ncolumn, nrow, files_per_panel=1)->bool:
    ...
    for filenumber in range(nfiles):
        ...
        if hasattr(newdata, 'dataset') and len(newdata.dataset) > 0:
            ...
            thisax = axs.flat[(filenumber // files_per_panel) % numaxs]
    ...
    for filenumber in range(nfiles):
        # Compute which panel (subplot) this file belongs to
        panel_index = filenumber // files_per_panel
        # Shortcut for the current axis
        thisax = axs.flat[panel_index % numaxs]
        ...
        if args.panels and files_per_panel == 1:
        ...
        if args.panels and numaxs > 1 and filenumber % files_per_panel == 0:
            sublabel = string.ascii_uppercase[panel_index] #Add letter to each subplot
            if args.debug:
                print(f"Setting subplot {panel_index+1} to {sublabel}\n")
        ...
        if args.sharelabel and args.panels:
            if args.panels == 'side':
                # For 'side' layout: plt.subplots(ncolumn, nrow) — ncolumn is the number of rows
                rowindex = panel_index // nrow
                columnindex = panel_index % nrow
                last_row = ncolumn-1  # last row index in the transposed grid
            else:  # 'top' and AxB grid formats
                rowindex = panel_index // ncolumn #counts rows by taking the division without rest
                columnindex = panel_index % ncolumn #counts columns by taking the rest
                last_row = nrow-1
            # Define what x- and y-labels to print
            if rowindex == last_row and columnindex == 0: #Last row and first column (only applies to lower left plot)
...
def _is_grid_format(panels_str):
    """Return True if panels_str is in AxB format (e.g., '2x3' or '1X2')."""
    return bool(re.match(r'^\d+[xX]\d+$', str(panels_str))) if panels_str else False
...
def plot(filenames, **kwargs):
    ...
    # Validate the panels argument
    if args.panels and args.panels not in ('top', 'side') and not _is_grid_format(args.panels):
        print(f"Unrecognized panels value '{args.panels}'. Use 'top', 'side', or 'AxB' (e.g., '1x2'). Will ignore -panels command.")
        args.panels = None
    ...
    if args.panels and len(args.filename) > 1:
        if _is_grid_format(args.panels):
            # User-specified AxB grid: A rows, B columns
            m = re.match(r'^(\d+)[xX](\d+)$', args.panels)
            nrow = int(m.group(1))
            ncolumn = int(m.group(2))
            nsubplots = nrow * ncolumn
            files_per_panel = math.ceil(nfiles / nsubplots) if nsubplots > 0 else 1
            xframe = args.xframe
            yframe = args.yframe
            fig, axs = plt.subplots(nrow, ncolumn, figsize=(xframe, yframe), constrained_layout=True)
        else:
    ...
    if args.follow:
        def animate(t):
            try:
                for ax in axs.flat: #to avoid stacking of plots 
                    ax.clear()
                process_plot(args, fig, axs, nfiles, ncolumn, nrow, files_per_panel)
            except Exception as e:
        ...
        foundData = process_plot(args, fig, axs, nfiles, ncolumn, nrow, files_per_panel)
