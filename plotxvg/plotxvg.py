...
""",
    ...
    col_x = "1"
    parser.add_argument("-csvx", "--csvx", help="Choose what x column you would like in a csv file. Default is column "+col_x, nargs="+", type=str, default=col_x)
    col_y = "3"
    parser.add_argument("-csvy", "--csvy", help="Choose what y column(s) you would like in a csv file. Default is column "+col_y, nargs="+", type=str, default=col_y)
...
def read_xvg(args, filename:str, residual:bool=False, filelabel:bool=False):
    ...
    with open(filename, "r") as inf:
        ...
        if is_csv:
            ...
            labels["xlabel"] = headers[int(args.csvx[0])]
            if len(args.csvy) == 1:
                labels["ylabel"] = headers[int(args.csvy[0])]
            elif len(args.csvy) > 1:
                labels["ylabel"] = headers[int(args.csvy[0])].split('(')[-1].split(')')[0]
                legends.append(headers[int(args.csvy[0])].split('(')[0])
                for yl in args.csvy[1:]:
                    labels["ylabel"] = labels["ylabel"]+", "+headers[int(yl)].split('(')[-1].split(')')[0]
                    legends.append(headers[int(yl)].split('(')[0])

            print(args.csvx, "\t", args.csvy)
            print(len(args.csvx)+len(args.csvy))
        ...
        for line in inf:
            ...
            if is_csv:
                numwords = len(args.csvx)+len(args.csvy)
                col.append(int(args.csvx[0]))
                for i in args.csvy:
                    col.append(int(i))
            else:
                numwords = len(w)
