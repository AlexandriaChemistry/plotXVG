...
Creating heatmaps in order to study probability densities and create free energy landscapes is also supported by plotXVG.
...
\end{verbatim}
Calculating Gibbs free energy from the density data is done by adding the \texttt{-gibbs} flag.
Gibbs free energy is calculated with the equation:
\begin{equation}
    G = k_{\mathrm{B}}T * \mathrm{ln}P/P_0
\end{equation}
where $P_0$ is the probability corresponding to the most populated bin, which by definition has $G$ = 0,
$k_\mathrm{B}$ is Boltzmann's constant and $T$ is the absolute temperature which can be specified with a \texttt{--temperature} flag.
