...
all_examples = [
    ...
    ("mult_panels", "Multiple panels, also showcases the use of the -sharelabel flag. Also note the possibility to apply user-defined titles, seen in panel C.\nTo combine files with pre-defined titles and files with user-defined titles, use None as placeholders for the files with pre-defined titles.", "act_xvgfiles/COULOMB.xvg act_xvgfiles/COULOMB.xvg act_xvgfiles/COULOMB.xvg act_xvgfiles/COULOMB.xvg", "-panels -sharelabel -title 'Potential Energy' 'Potential Energy' None None"),
    ("vary_panel_content", "Panels that shows differently expressed data, such as two with lines and two with markers.\nFont- line- or marker-sizing are dynamic based on the number of subplot columns, but specified at will, by \nfor example adding -mksize 20 -mkwidth 4 in a subplot of two columns.", "gmx_xvgfiles/rmsd_calpha.xvg gmx_xvgfiles/temp_press.xvg gmx_xvgfiles/gyrate.xvg gmx_xvgfiles/2dproj_PC1_PC2.xvg", "-panels -ls solid solid solid solid dotted dashdot dashed None -mk None None None o + x ^ + -mksize 20 -mkwidth 4"),
    ...
    ("equalaxes", "Demonstrates the equal axes flag. This flag makes the plot square with equally large axes.", "act_xvgfiles/COULOMB.xvg", "-eqax"),
    ...
    ("stats","Shows statistics (RMSD, R2).", "act_xvgfiles/COULOMB.xvg act_xvgfiles/EPOT.xvg", "-stats -panels"),
    ("res", "Plots the residual of the data, meaning x is substracted from y for all data sets.\nStatistics are based on original train and test values and will not be affected by the residual flag.", "act_xvgfiles/COULOMB.xvg act_xvgfiles/EPOT.xvg", "-stats -panels -res"),
